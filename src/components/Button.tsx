...
const Button = ({ children, className, type = "button", size = "medium", disabled, startIcon, endIcon, onClick, variant = "primary" }: ButtonProps) => {
  return (
    <button
      className={classNames("inline-flex items-center justify-center gap-x-1.5 rounded-full font-semibold text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 transition-all duration-500 ease-in-out",
        variant === "error-icon" ? "" : "shadow-md",
        variant === "primary" ? "bg-primary-700 hover:bg-primary-600 focus-visible:outline-primary-600" : 
        ...
        variant === "error-icon" ? "text-red-600 hover:text-red-400 shadow-none bg-transparent" :
          "bg-gray-50 text-primary-700 hover:bg-gray-100 border border-primary-600",
